...
class GestionStagiaire {
    ...
    public function editerStagiaire($nom,$prenom,$email,$ville,$FINDemail){
        $queryVille = "SELECT id FROM ville WHERE nom_ville = :ville";
        $stmtVille = $this->pdo->prepare($queryVille);
        ...
        $queryPersonne = "UPDATE personne SET nom = :nom , prenom = :prenom , id_ville = :villeID WHERE id IN (SELECT id_personne FROM stagiaire WHERE email = :FINDemail)";
        $stmtPersonne = $this->pdo->prepare($queryPersonne);
        $stmtPersonne->bindParam(':nom',$nom);
        $stmtPersonne->bindParam(':prenom',$prenom);
        $stmtPersonne->bindParam(':villeID',$id_ville['id']);
    ...
    {
        $queryGraphique = "SELECT ville.nom_ville, COUNT(stagiaire.id) AS nombre_stagiaires FROM ville INNER JOIN personne ON personne.id_ville = ville.id INNER JOIN stagiaire ON stagiaire.id_personne = personne.id GROUP BY ville.nom_ville";
